...
For example, the user `GaroxXiz` must create `GaroxXiz/GaroxXiz`.
...
Clone the profile repository you just created. For example, if your username is `GaroxXiz`:

```bash
git clone https://github.com/GaroxXiz/GaroxXiz.git
cd GaroxXiz
npm ci
```

Replace every `GaroxXiz` with your own GitHub username. `npm ci` installs the exact dependency versions recorded in the included lockfile.
